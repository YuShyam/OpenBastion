...
> **核心原則**: 標準 RFC 4253 協議、零假資料、終端防破版、僅追加審計。
...
|    - 清理資料庫中殘留的異常狀態                                   |
+-------------------------------------------------------------------+
| 6. 僅追加審計紀錄 (Append-Only Audit Trail)                         |
|    - 記錄登入、連線建立、指令執行與斷線事件                       |
|    - 支援標準日誌格式輸出 (RFC 5424)                              |
+-------------------------------------------------------------------+
```
...
- **內建驅動**:
  ...
  - `SQLiteStorage`: 關聯式儲存，**強制啟用 WAL 模式**，支援多執行緒並行讀寫，避免在高頻審計寫入時發生資料庫鎖定。
...
- **儲存規範**: 資料庫與設定檔嚴禁明文存放目標主機的私鑰與密碼。
- **加密與硬體綁定**:
  - 採用 AES-256-GCM 進行對稱加密。
  - 金鑰衍生結合本地主金鑰與主機硬體特徵（Linux `/etc/machine-id` 或 Windows `MachineGuid`），防止資料庫檔案被拷貝至未授權設備後遭離線解密。
- **備份與遷移 (匯出/匯入)**:
  - 支援管理員命令列匯出：輸入指定備份密碼，將憑證解密後重新封裝為加密備份檔（PBKDF2-SHA256 搭配 AES-256-GCM）。
  - 支援跨機匯入：在新設備輸入備份密碼解鎖，並以新主機之硬體特徵重新加密寫入資料庫。
